...
DEMOGRAPHICS = ["AGE", "SEX", "EDUCATION", "BMI", "WEIGHT", "HEIGHT"]
VASCULAR = ["HTN", "DM2", "CAD_01", "PREV_CHD2", "AFIBPREVALENT_ECG", "ATRFIBRFLUTTER_ANY"]
LIFESTYLE = ["SMOKINGSTATUS", "SMOKING_YEARS", "EVERSMOKER", "ALCOHOLGWEEK", "ALCOHOLGWEEK_MID", "PHYSACTIVPRES", "PHYSACTIVALL"]
SVD_MARKERS = ["MICROBLD", "HEMATOMA", "SUBCORTYN", "VRSYN", "INFCORTYN", "INFCERYN"]
WMH_LOBAR = ["OccipitalD", "ParietalD", "TemporalD", "FrontalD", "OccipitalP", "ParietalP", "TemporalP", "FrontalP"]
BASELINE_COGNITION = ["SA1MEMORY2", "SA1WORKING2", "SA1SPEED2"]
REGIONAL_DAWM = ["FrontalDAWM", "TemporalDAWM", "ParietalDAWM", "OccipitalDAWM"]

LEAKAGE_PREFIXES = (
    "DEM_", "DEMENTIA", "COGSTATUS", "SA2", "DAYS_AGES_AGESII",
    "deltaMemory", "deltaExec", "deltaSpeed", "AGE_DEM_FU",
    "SplitWMH", "hiWMH", "loWMH",
)

CATEGORICAL_FEATURES = {
    "SEX", "HTN", "DM2", "CAD_01", "PREV_CHD2", "AFIBPREVALENT_ECG",
    "ATRFIBRFLUTTER_ANY", "SMOKINGSTATUS", "EVERSMOKER", "PHYSACTIVPRES",
    "PHYSACTIVALL", "MICROBLD", "HEMATOMA", "SUBCORTYN", "VRSYN",
    "INFCORTYN", "INFCERYN", "EDUCATION",
}
...
def prepare_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "baselineICV_A2" not in df and {"A2_B_BPP01GM_VOL", "A2_B_BPP01WM_VOL", "A2_B_BPP01CSF_VOL"}.issubset(df.columns):
        df["baselineICV_A2"] = df[["A2_B_BPP01GM_VOL", "A2_B_BPP01WM_VOL", "A2_B_BPP01CSF_VOL"]].sum(axis=1)
...
def build_feature_sets(df: pd.DataFrame) -> dict[str, list[str]]:
    ...
    base = (
        ...
        + wmh
        + _available(columns, ["baselineICV", "baselineICV_A2"])
